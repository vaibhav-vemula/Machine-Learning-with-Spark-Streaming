...
def sendCIFARBatchFileToSpark(tcp_connection, input_batch_file):
    ...
    with open(f'../dataset/cifar/{input_batch_file}', 'rb') as batch_file:
...
def sendPokemonBatchFileToSpark(tcp_connection, input_batch_file):
    # load the entire dataset
    with open(f'../dataset/pokemon/{input_batch_file}.pickle', 'rb') as batch_file:
...
def streamDataset(tcp_connection, dataset_type):    # function to stream a dataset
    ...
    for dataset in DATASETS:
        streamCSVFile(tcp_connection, f'../dataset/{dataset_type}/{dataset}.csv')
        time.sleep(1)
